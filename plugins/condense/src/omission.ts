...
function objectsDir(): string { return join(dataRoot(), "objects"); }
...
export async function saveManifest(sessionId: string, contentIds: string[]): Promise<void> {
  await atomicJson(manifestPath(sessionId), { version: 3, sessionId, contentIds: [...new Set(contentIds)].sort() } satisfies OmissionManifest);
}
...
export async function loadManifest(sessionId: string): Promise<OmissionManifest | null> {
  ...
  if (!isRecord(value) || (value["version"] !== undefined && value["version"] !== 3) || value["sessionId"] !== sessionId || !Array.isArray(value["contentIds"]) || value["contentIds"].some((id) => typeof id !== "string")) {
...
function boundedMetadata(metadata: Record<string, unknown>): Record<string, unknown> {
  ...
  for (const [key, value] of Object.entries(metadata)) {
    ...
    else if (Array.isArray(value)) result[key] = value.slice(0, 20).map((item) => typeof item === "string" ? item.slice(0, 120) : item);
  }
  return result;
}
...
const LEGACY_SKILL_NOTICE = /^\[Skill "[^"]+" output \(\d+ chars\) omitted by condense\. Re-invoke \/[^;\]]+; fallback retrieve: ([0-9a-fA-F]{12}:omitted-\d+)\]$/;
...
async function resolveEntry(contentId: string): Promise<ResolvedEntry | null> {
  if (/^c3_[A-Za-z0-9_-]{22}$/.test(contentId)) {
    ...
    if (value === null) return null;
    if (!isRecord(value) || value["version"] !== 3 || value["id"] !== contentId
      || typeof value["kind"] !== "string" || !("value" in value)
      || !isRecord(value["metadata"]) || typeof value["renderedLength"] !== "number"
    ...
    if (renderOmissionValue(object.value).length !== object.renderedLength) throw new Error(`Rendered-length check failed for Content-ID ${contentId}`);
    return { contentId, legacy: false, version: 3, entry: { kind: object.kind, value: object.value, metadata: object.metadata, renderedLength: object.renderedLength, sha256: object.sha256 } };
  }
  ...
  for (const path of paths) {
    ...
    if (isV2(value) && value.entries[contentId]) found.push({ contentId, entry: value.entries[contentId]!, legacy: false, version: 2 });
    else if (isV1(value) && value.entries[contentId]) {
      const content = value.entries[contentId]!.content;
      found.push({ contentId, legacy: true, version: 1, entry: { kind: "tool-output", value: content, metadata: { legacy: true }, renderedLength: content.length, sha256: sha256(stableStringify(content)) } });
    }
  }
...
export async function readOmittedContent(contentId: string, options: { start?: number; length?: number; config: CondenseConfig }) {
  ...
  if (!Number.isInteger(requestedLength) || requestedLength < 1 || requestedLength > options.config.retrieval.maxReadChars) throw new Error(`length must be between 1 and ${options.config.retrieval.maxReadChars}`);
  ...
  const metadata = metadataText.length <= 2000 ? resolved.entry.metadata : { truncated: true, preview: metadataText.slice(0, 1900) };
  return { contentId, kind: resolved.entry.kind, metadata, sha256: resolved.entry.sha256, totalLength: rendered.length, start, end, nextStart: end < rendered.length ? end : null, truncated: end < rendered.length, text: rendered.slice(start, end), legacy: resolved.legacy };
}
...
export async function searchOmittedContent(args: {
  ...
  if (args.query.length > config.maxRegexPatternChars) throw new Error(`query exceeds ${config.maxRegexPatternChars} characters`);
  ...
  const addResult = (result: JsonRecord): boolean => {
    ...
    if (responseChars + serialized > config.maxResponseChars) { responseTruncated = true; return false; }
    responseChars += serialized; results.push(result); return true;
  };
  for (const contentId of [...new Set(ids)]) {
    const resolved = await resolveEntry(contentId);
    if (!resolved) { missingContentIds.push(contentId); continue; }
    ...
    if (mode === "literal") {
      ...
      while (results.length < maxMatches && matcher.find()) {
        const start = matcher.start(); const end = matcher.end();
        const rawMatch = text.slice(start, end); const match = rawMatch.slice(0, 500);
        if (!addResult({ contentId, kind: resolved.entry.kind, start, end, line: lineNumberAt(text, start), match, matchLength: rawMatch.length, matchTruncated: match.length < rawMatch.length, captures: [], ...contextFor(text, start, end, contextLines, config.maxExcerptChars) })) break;
      }
    } else {
      const flags = (caseSensitive ? 0 : RE2JS.CASE_INSENSITIVE) | RE2JS.MULTILINE;
      ...
      while (results.length < maxMatches && matcher.find()) {
        ...
        if (!addResult({ contentId, kind: resolved.entry.kind, start, end, line: lineNumberAt(text, start), match, matchLength: rawMatch.length, matchTruncated: match.length < rawMatch.length, captures: boundedCaptures, capturesTruncated: captures.some((capture, i) => capture !== null && capture.length > (boundedCaptures[i]?.length ?? 0)), ...contextFor(text, start, end, contextLines, config.maxExcerptChars) })) break;
      }
    }
  }
  return { query: args.query, mode, complete: missingContentIds.length === 0 && !responseTruncated, searchedContentIds: ids.length - missingContentIds.length, missingContentIds, responseTruncated, matches: results };
}

function idsInNotice(text: string): string[] { const notice = parseOmissionNotice(text); return notice ? [notice.contentId] : []; }
