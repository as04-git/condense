...
test("finds the current operation once and returns its preceding cutoff", () => {
  const rows = [
    ...
    row({ type: "user", uuid: "skill", parentUuid: "op", isMeta: true, message: { content: [{ type: "text", text: "Base directory for this skill: /x/skills/condense" }] } }),
    row({ type: "assistant", uuid: "call", parentUuid: "skill", message: { content: [{ type: "tool_use", id: "tool1", name: "Bash", input: { command: "bun /x/condense/src/condense.ts analyze" } }] } }),
  ];
  expect(findCondenseOperationBoundary(rows)).toEqual({ operationUserUuid: "op", cutoffUuid: "a1" });
  expect(() => validateCondenseSuffix([...rows, row({ type: "user", uuid: "late", parentUuid: "call", message: { content: "new work" } })], "a1")).toThrow("real user message");
});
...
test("condense markers never become semantic turns", () => {
  ...
  expect(turns).toHaveLength(1); expect(turns[0]!.userRows[0]!.uuid).toBe("u2");
});

test("fails closed when the SDK omits fork lineage metadata", () => {
  expect(() => assertForkLineage([row({ type: "user", uuid: "u", parentUuid: null, message: { content: "x" } })])).toThrow("forkedFrom.messageUuid");
});
...
test("large task notifications and errors are rankable with raw flags", () => {
  ...
  const rows = [
    ...
    row({ type: "assistant", uuid: "a1", parentUuid: "u1", message: { content: [{ type: "tool_use", id: "agent1", name: "Agent", input: { prompt: "work" } }, { type: "tool_use", id: "bash1", name: "Bash", input: { command: "test" } }] } }),
    row({ type: "user", uuid: "r1", parentUuid: "a1", message: { content: [{ type: "tool_result", tool_use_id: "agent1", content: task }, { type: "tool_result", tool_use_id: "bash1", content: error, is_error: true }] } }),
  ];
...
test("Bash repeats are not newer-target signals while file reads are", () => {
  ...
  const rows = [
    ...
    row({ type: "assistant", uuid: "a1", parentUuid: "u1", message: { content: [{ type: "tool_use", id: "b1", name: "Bash", input: { command: "git status" } }, { type: "tool_use", id: "f1", name: "Read", input: { file_path: "/x" } }] } }),
    row({ type: "user", uuid: "r1", parentUuid: "a1", message: { content: [{ type: "tool_result", tool_use_id: "b1", content: huge }, { type: "tool_result", tool_use_id: "f1", content: huge }] } }),
    row({ type: "user", uuid: "u2", parentUuid: "r1", message: { content: "second" } }),
    row({ type: "assistant", uuid: "a2", parentUuid: "u2", message: { content: [{ type: "tool_use", id: "b2", name: "Bash", input: { command: "git status" } }, { type: "tool_use", id: "f2", name: "Read", input: { file_path: "/x" } }] } }),
    row({ type: "user", uuid: "r2", parentUuid: "a2", message: { content: [{ type: "tool_result", tool_use_id: "b2", content: huge }, { type: "tool_result", tool_use_id: "f2", content: huge }] } }),
  ];
